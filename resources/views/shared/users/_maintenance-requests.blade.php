@forelse ($maintenanceRequests as $request)
	<a href="{{ is_null($request->type) && $request->user_id === Auth::id() ? route('maintenance.editPost', $request->id) : (Auth::user()->type != 0 ? '#' : route('maintenance.edit', $request->id)) }}" class="list-group-item list-group-item-action flex-column align-items-start">
